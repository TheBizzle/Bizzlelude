...
import Data.Eq(Eq((==), (/=)))
import Data.Foldable(and, any, all, Foldable(elem, fold, foldMap, foldr, foldr', foldl, foldl', foldr1, foldl1, null, maximum, minimum, sum, product), find, foldlM, foldrM, for_, maximumBy, minimumBy, or, sequenceA_)
...
import GHC.Real((^), (^^), Fractional(recip, fromRational), fromIntegral, Integral(quot, rem, div, mod, quotRem, divMod, toInteger), RealFrac(properFraction, truncate, round, ceiling, floor), Real(toRational), realToFrac)
import GHC.Show(Show(show))

import Numeric(Floating, pi, exp, log, sqrt, (**), logBase, sin, cos, tan, asin, acos, atan, sinh, cosh, tanh, asinh, acosh, atanh, showFFloat)
